...
dx = x(2)-x(1); % differece between x intervals
xm = x(1:nx-1) + 0.5*dx;

% ni = x < l/2; % one half of material is n-type
% pi = x >= l/2; % other half of material is p-type

Nd = 4e16 * 1e6; % Const. 1/cm3 (100 cm/m)^3
Na = 1e16 * 1e6;
NetDoping = linspace(-Na,Nd,nx);

% half of the material is n-type, half way through, material immediately
% switches to p-type
% NetDoping(ni) = Nd;
% NetDoping(pi) = -Na;
